...
| pytest / vitest plugin | `evals4j-junit5` | Assertions, `EvalReport`, and `@EvalSuite` — a JUnit `Extension` that supplies the report and writes it out, with `META-INF/services` registration for a module that wants no annotation. Under `@EvalSuite` the report attaches itself through an ambient `EvalScope`, reaching what the pytest plugin gets by patching itself in — without the patching. |
